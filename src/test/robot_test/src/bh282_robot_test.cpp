...
void jointsTrajectory(arma::vec qT, double total_time)
{
  ...
  std::vector<bhand_::JointName> jn = {bhand_::SPREAD, bhand_::FING1, bhand_::FING2, bhand_::FING3};
...
void jointPositionControl(arma::vec qT, double total_time)
{
  ...
  arma::vec q(N_joints); // {SPREAD, FING1, FING2, FING3}
  ...
  while (arma::norm(qT-q)>err_thres && bh282_robot->isOk())
  {
    ...
    bh282_robot->setJointPosition(q_ref, {bhand_::SPREAD, bhand_::FING1, bhand_::FING2, bhand_::FING3});
    bh282_robot->update();
    for (int i=0;i<N_joints;i++) q(i) = bh282_robot->getJointPosition((bhand_::JointName)i);
  }
  if (!bh282_robot->isOk())
...
void jointVelocityControl(arma::vec qT, double total_time)
{
  ...
  while (arma::norm(qT-q)>err_thres && bh282_robot->isOk())
  {
    ...
    // bh282_robot->setJointVelocity(dq_cmd, {bhand_::SPREAD, bhand_::FING1, bhand_::FING2, bhand_::FING3});
    bh282_robot->update();
    for (int i=0;i<N_joints;i++) q(i) = bh282_robot->getJointPosition((bhand_::JointName)i);
  }
  if (!bh282_robot->isOk())
