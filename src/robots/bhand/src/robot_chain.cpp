...
void RobotChain::updateState()
{
  ...
  arma::vec j_pos = getJointsVelocity();
  joint_vel = (j_pos - joint_prev_pos) / ctrl_cycle;

  pose = getTaskPose(j_pos);
  Jrobot = getJacobian(j_pos);
...
void RobotChain::stop()
{
  ...
  joint_prev_pos = getJointsPosition();
  updateState();
  // mode = bhand_::Mode::IDLE;
  // print_info_msg("Mode changed to \"" + getModeName() + "\"\n");
}
...
void RobotChain::setJointPositionHelper(const arma::vec &j_pos)
{
  ...
  joint_prev_pos = current_j_pos;
  joint_pos = j_pos;
}

void RobotChain::setJointVelocityHelper(const arma::vec &j_vel)
{
  setJointPositionHelper(getJointsPosition() + j_vel*ctrl_cycle);
}

void RobotChain::setTaskVelocityHelper(const arma::vec &task_vel)
{
  setJointVelocityHelper(arma::solve(getJacobian(),task_vel));
}
